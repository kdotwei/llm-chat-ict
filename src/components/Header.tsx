    ...
    <header className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 truncate text-xs text-gray-400 dark:text-gray-500">
          <span title={model} className="truncate max-w-[120px] sm:max-w-none">
            Base: {model || '—'}
          </span>
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              {provider === 'lm-studio' ? 'LM Studio' : provider === 'openai' ? 'OpenAI Cloud' : 'Custom Endpoint'}{' '}
              {isConnected ? 'Connected' : 'Offline'}
            </span>
          </div>
        </div>
        <div className="mt-1.5 flex flex-wrap gap-1 text-[10px] uppercase tracking-wide">
      ...
      <h1 className="px-3 text-center text-lg font-semibold tracking-tight text-gray-800 dark:text-gray-100 font-heading">LLM Chatroom v2</h1>
